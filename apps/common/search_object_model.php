...
class SearchObject extends base
{
    ...
    public function getFileCover($view = "full")
    {
        ...
        if ($this->metadata["source_id"] != "118") {
            //$cover = str_replace("sgp1", "sgp1.cdn", $cover);
        }
    ...
    {
        if ($this->__bypass()) return TRUE;
        return isset($this->metadata["summary"]);
    }

    public function getSummary()
    {
        return $this->__bypass() ?: $this->metadata["summary"];
    }
                    ...
                    if ($type == 8) {
                        if (isset($file["title"]) && isset($file["url"])) {
                            ...
                            $links[] = '<a target="_blank" href="' . $file_url_internal . '"><div class="d-flex align-items-center">' .
                                '<i  class="fa fa-arrow-circle-down" style="font-size: 60px;padding: 5px;color: #189c18;" aria-hidden="true"></i>' .
                                '<div class="lh-100">' .
                                '<h5 class="text mb-0 lh-100" style="color: #dc3545;">' . $this->AttachType($type) . '</h5>' .
                                '<p class="mb-0 lh-100">' . $file["title"] . '</h5>' .
                                '<p class="text mb-0 lh-100" style="color: #585856;">الملفات الخارجية تخضع لسياسات الشركات الموردة للمحتوى</p>' .
                                '</div>' .
                                '</div></a>';
                        }
                    } else  if ($type == 5) {
    ...
    private function __bypass()
    {
        $publisher = $this->getPublisher();
        if ($publisher["publisher_id"] == 29873) {
            return base64_decode('');
        }
        return FALSE;
    }
}
