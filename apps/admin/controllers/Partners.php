...
class Partners extends MY_Controller {
    ...
    public function index(){
        ...
        if( can("edit_sections") ){
            ...
            $this->db->select("partners.*, sec_title_".$this->locale." as title");
            $sections = $this->db->order_by("sec_order","ASC")->get("partners")->result();

            $data["data"]["sections"] = $sections;
            $data["views"]["content"] = 'partners/list_sections';
    ...
    public function add_section(){
        ...
        if( can("edit_sections") ){
            ...
            $data["views"]["content"] = 'partners/forms/add_edit_section';
    ...
    public function insert_section(){
        ...
        if( can("edit_sections") ){
            ...
            redirect(base_url(admin_base."partners/"));
    ...
    public function edit_section($sec_id){
        ...
        if( can("edit_sections") ){
            ...
            $section = $this->db->limit(1)->get("partners")->row();
    ...
    public function update_section(){
        ...
        if( can("edit_sections") ){
            ...
            $this->db->where("sec_id",$sec_id)->update("partners",$data);
    ...
    public function widgets($sec_id){
        ...
        if( can("edit_sections") ){
            ...
            $widgets = $this->db->order_by("widget_order","ASC")->get("partners_widgets")->result();
            if($widgets){
                $w_ids = get_ids_array($widgets,"widget_id");

                $this->db->select("item_class,item_id,item_order,item_widget_id");
                $items = $this->db->where_in("item_widget_id",$w_ids)->order_by("item_order","ASC")->get("partners_widget_items")->result();
                if($items){
                    $i_ids = get_ids_array($items,"item_id");

                    $this->db->select("field_item_id, field_name as name, field_value_".$this->locale." as value");
                    $fields = $this->db->where_in("field_item_id",$i_ids)->get("partners_widget_fields")->result();
            ...
            $classes = $this->db->order_by("class_id","DESC")->get("partners_content_classes")->result();
            ...
            $data["views"]["content"] = 'partners/widgets';
    ...
    public function add_widget($sec_id){
        ...
        if( can("edit_sections") ){
            ...
            $this->db->select("partners.*, sec_title_".$this->locale." as title");
            $sections = $this->db->order_by("sec_order","DESC")->get("partners")->result();

            $data["data"]["sections"] = $sections;
            $data["data"]["sec_id"] = $sec_id;
            $data["views"]["content"] = 'partners/forms/add_edit_widget';
    ...
    public function insert_widget(){
        ...
        if( can("edit_sections") ){
            ...
            $this->db->insert("partners_widgets",$data);

            redirect(base_url(admin_base."partners/widgets/".$widget_sec_id));
    ...
    public function edit_widget($widget_id){
        ...
        if( can("edit_sections") ){
            ...
            $widget = $this->db->where("widget_id",$widget_id)->limit(1)->get("partners_widgets")->row();
    ...
    public function update_widget(){
        ...
        if( can("edit_sections") ){
            ...
            $this->db->where("widget_id",$widget_id)->update("partners_widgets",$data);
    ...
    public function delete_widget($widget_id,$redirect = TRUE){
        ...
        if( can("edit_sections") ){
            ...
            $items = $this->db->where("item_widget_id",$widget_id)->get("partners_widget_items")->result();

            foreach($items as $item){
                $this->db->where("item_id",$item->item_id)->limit(1)->delete("partners_widget_items");
                $this->db->where("field_item_id",$item->item_id)->delete("partners_widget_fields");
            }

            $this->db->where("widget_id",$widget_id)->limit(1)->delete("partners_widgets");
    ...
    public function add_item($class,$widget_id){
        ...
        if( can("edit_sections") ){
            ...
            $fields = $this->db->where("f_class_name",$class)->order_by("f_order")->get("partners_content_classes_fields")->result();
            ...
            $data["views"]["content"] = 'partners/forms/add_edit_item';
    ...
    public function insert_item(){
        ...
        if( can("edit_sections") ){
            ...
            $this->db->insert("partners_widget_items",$data);

            $item_id = $this->db->insert_id();


            $fields = $this->db->where("f_class_name",$item_class)->order_by("f_order")->get("partners_content_classes_fields")->result();
            ...
            foreach($fields as $field){
                ...
                $this->db->insert("partners_widget_fields",$data);
    ...
    public function edit_item($item_id){
        ...
        if( can("edit_sections") ){
            ...
            $item = $this->db->where("item_id",$item_id)->limit(1)->get("partners_widget_items")->row();

            $fields = $this->db->where("f_class_name",$item->item_class)->order_by("f_order")->get("partners_content_classes_fields")->result();

            $_myfields = $this->db->where("field_item_id",$item->item_id)->get("partners_widget_fields")->result();

            $widget = $this->db->where("widget_id",$item->item_widget_id)->limit(1)->get("partners_widgets")->row();
    ...
    public function update_item(){
        ...
        if( can("edit_sections") ){
            ...
            $this->db->where("item_id",$item_id)->update("partners_widget_items",$data);
            ...
            foreach($fields as $field){
                ...
                $field = $this->db->get("partners_widget_fields")->row();

                if($field){
                    $this->db->where("field_id",$field->field_id);
                    $field = $this->db->update("partners_widget_fields",$data);
                }else{
    ...
    public function delete_item($item_id,$redirect = TRUE){
        ...
        if( can("edit_sections") ){
            ...
            $this->db->where("item_id",$item_id)->limit(1)->delete("partners_widget_items");

            $this->db->where("field_item_id",$item_id)->delete("partners_widget_fields");
    ...
    public function class_fields($class){
        ...
        if( can("edit_sections_classes") ){
            ...
            $fields = $this->db->order_by("f_order","ASC")->get("partners_content_classes_fields")->result();

            $data["data"]["class"] = $class;
            $data["data"]["fields"] = $fields;
            $data["views"]["content"] = 'partners/class_fields';
    ...
    public function add_class(){
        ...
        if( can("edit_sections_classes") ){
            ...
            $data["views"]["content"] = 'partners/forms/add_edit_class';
    ...
    public function insert_class(){
        ...
        if( can("edit_sections_classes") ){
            ...
            $exists = $this->db->where("class_name",$class_name)->limit(1)->get("partners_content_classes")->row();
            ...
            if(!$exists){
                ...
                $this->db->insert("partners_content_classes",$data);

            }

            redirect(base_url(admin_base."partners/class_fields/".$class_name));
    ...
    public function edit_class($class_id){
        ...
        if( can("edit_sections_classes") ){
            ...
            $class = $this->db->limit(1)->get("partners_content_classes")->row();
    ...
    public function update_class(){
        ...
        if( can("edit_sections_classes") ){
            ...
            $class = $this->db->where("class_id",$class_id)->limit(1)->get("partners_content_classes")->row();

            $exists = $this->db->where("class_id !=",$class_id)->where("class_name",$class_name)->limit(1)->get("partners_content_classes")->row();
            ...
            if(!$exists){
                ...
                $this->db->where("class_id",$class_id)->update("partners_content_classes",$data);


                $data = array(
                    "f_class_name" => trim($class_name),
                );
                $this->db->where("f_class_name",$class->class_name)->update("partners_content_classes_fields",$data);
    ...
    public function add_class_field($class){
        ...
        if( can("edit_sections_classes") ){
            ...
            $data["data"]["class"] = $class;
            $data["views"]["content"] = 'partners/forms/add_edit_class_field';
    ...
    public function insert_class_field(){
        ...
        if( can("edit_sections_classes") ){
            ...
            $exists = $this->db->where("f_name",$f_name)->where("f_class_name",$f_class_name)->limit(1)->get("partners_content_classes_fields")->row();
            ...
            if(!$exists){
                ...
                $this->db->insert("partners_content_classes_fields",$data);

            }

            redirect(base_url(admin_base."partners/class_fields/".$f_class_name));
    ...
    public function edit_class_field($f_id){
        ...
        if( can("edit_sections_classes") ){
            ...
            $field = $this->db->limit(1)->get("partners_content_classes_fields")->row();
    ...
    public function update_class_field(){
        ...
        if( can("edit_sections_classes") ){
            ...
            $exists = $this->db->limit(1)->get("partners_content_classes_fields")->row();
            ...
            if(!$exists){
                ...
                $this->db->where("f_id",$f_id)->update("partners_content_classes_fields",$data);
    ...
    public function delete_class_field($f_id){
        ...
        if( can("edit_sections_classes") ){
            ...
            $this->db->where("f_id",$f_id);
            $this->db->limit(1)->delete("partners_content_classes_fields");

            redirect(base_url(admin_base."partners/class_fields/".$field->f_class_name));
